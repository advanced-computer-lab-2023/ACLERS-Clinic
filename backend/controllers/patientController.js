...
const setAppointment = asyncHandler(async (req, res) => {
    ...
    if (date < now) {
        ...
        var patientHealthRecord = await PatientHealthRecord.findOne({patient:patientId})
        ...
        var registeredPatients = await RegisteredPatients.findOne({doctor:doctorId})
        
        if(registeredPatients){

            var isPatientAlreadyAdded = registeredPatients.patients.includes(patientHealthRecordId);
    ...
    console.log(`Patient with ID ${patientIdToAdd} added to RegisteredPatients.`);
  } 
...
module.exports = {searchForDoctor,selectPresc,viewMyPerscriptions, addFamilyMember, viewFamilyMembers, setAppointment, filterAppointments, viewDoctors, viewHealthPackages, subscribeHealthPackage, viewDoctor }